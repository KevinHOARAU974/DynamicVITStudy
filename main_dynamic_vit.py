...
from define import define_model_teacher, define_criterion
from dataset import create_data_loaders, create_splits
...
def get_args_parser():
    ...
    parser.add_argument('--distill', default=True, type=utils.str2bool,
                        help="Allow distillation during the training")
    parser.add_argument('--kl', default=True, type=utils.str2bool,
                        help="Allow kl part of the loss")
    
    #Arguments relative to the split of dataset
    parser.add_argument('--raw_dataset_path', default='data/imagenet', type=str, help='Dataset path')
    parser.add_argument("--split_data", type=bool, default=False, help="If split dataset is needed")
    parser.add_argument("--train_pct", type=float, default=0.7, help="Ratio of train data")
    parser.add_argument("--val_pct", type=float, default=0.1, help="Ratio of validation data")
    
    #Argument relative to trained
    parser.add_argument("--pretrained_teacher_model", type=str, default='vit_b_16.pth', help="Path to the teacher model")
    parser.add_argument("--pretrained_student_model", type=str, default='/content/checkpoint-best.pth', help="Path to the strudent model")
...
def append_trial_to_yaml(new_trial_data, filename="dynamicVit_results.yaml"):
    ...
    existing_data.append(new_trial_data)

    # On écrase avec le nouveau contenu combiné
    with open(filename, "w") as f:
        yaml.dump(existing_data, f)

def apply_dynamicVit(args):
...
def main(args):
    ...
    if args.split_data:
        create_splits(args.raw_dataset_path, args.train_pct, args.val_pct)
        
    train_loader, val_loader, test_loader = create_data_loaders("./", args.batch_size, args.input_size)
    device = "cuda" if torch.cuda.is_available() else 'cpu'
    example_input = torch.randn(args.val_batch_size, 3, args.img_size, args.img_size).to(device)
    ...
    result_loss_teacher = trainer.test(teacher_pl, test_loader)
    result_loss_model = trainer.test(model_pl, test_loader)
    
    results_benchmark_teacher =  benchmark(teacher_pl, example_input, gpu_only=True)
    ...
    new_entry = {
        ...
        "test_dynamicVit": result_loss_model,
        "benchmark_dynamicVit": results_benchmark_model,
    }
    append_trial_to_yaml(new_entry, filename=f"{args.model}.yaml")
